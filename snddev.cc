...
//
ModemSoundDevice::ModemSoundDevice(const std::string &mode, size_t id, size_t rate, size_t win, const std::string &ext) :
		SoundCard(id, rate, 1, win),
		m_Filter(0), m_Current(0), m_Decoding(0), m_MFSK(0), m_Keep(false), m_Start(0) {
	...
	tempDirs.push_back("/var/run/"); // this should be on the same ramdisk
	tempDirs.push_back("/tmp/");     // this is probably on a real drive
	tempDirs.push_back("/var/tmp/"); // this is probably on a real drive
	for (std::deque<std::string>::const_iterator tp = tempDirs.begin(); tp != tempDirs.end(); ++tp) {
		if (KK5JY::Path::isDir(*tp) || KK5JY::Path::isLink(*tp)) {
			m_TempDir = *tp;
			break;
		}
	}

	// make sure something sane was found
	...
	if (m_Rate == 12000) {
		...
		m_Decimator = new KK5JY::DSP::DecimatorLI<float, float>(
			rate, // sound card rate
			12000 // 'jt9' rate
		);
	} else {
		// decimator for evenly divisiable rate
	...
	if (m_Mode == "FT8") {
		...
		m_FrameStart = 59.8;
		m_FrameEnd = 52.0;
		m_Lead = 1.0 * m_Rate; // 1s lead-in for JT65
		...
		m_FrameStart = 59.8;
		m_FrameEnd = 52.0;
		m_Lead = 1.0 * m_Rate; // 1s lead-in for JT9
		...
		m_FrameStart = 119.9;
		m_FrameEnd = 112.0;
		m_Lead = 1.0 * m_Rate; // 1s lead-in for WSPR
	...
	if (decoding) {
		...
		if ((decoding->getDecodes(buffer) > 0) && ! m_Purge) {
			...
			for (i = buffer.begin(); i != buffer.end(); ++i) {
				...
				KK5JY::FT8::send_message("D",
					std::to_string(static_cast<time_t>(when)) + " " + (i->substr(m_TrimIdx)));
			}
		}
...
bool ModemSoundDevice::cancelTransmit() {
	...
	return m_MFSK != 0; // return true if there was anything to cancel
}
...
void ModemSoundDevice::event(float *in, float *out, size_t count) {
	...
	if (decoder) {
		...
		if (sec > (m_FrameEnd + m_Trail) && sec < m_FrameStart) {
			...
			// if the prior decode has not completed...
			if (m_Decoding) {
				...
				KK5JY::FT8::send_trace("End capture of " + std::to_string(window) + " msec; " +
					KK5JY::FT8::timestring(m_Start) + " to " + KK5JY::FT8::timestring(stop));
			}

			// report the highest input level
			if (m_Intervals++) { // this skips the first timeslot
				int db = (m_MaxInput == 0) ? minimum_db : KK5JY::DSP::decibels(m_MaxInput);
				if (db > 0) db = 0;
		...
		if (sec >= m_FrameStart || sec < m_FrameEnd) {
			...
			std::string name = m_TempDir + "000101_" + pbuf;

			// create a new decoder and configure it
			decoder = m_Current = new KK5JY::FT8::Decode<float>(m_Mode, name, m_TempDir, KK5JY::FT8::abstime(), m_Depth);
	...
	if (inWindow) {
		...
		if ( ! thisSlot) {
			...
			/*KK5JY::FT8::send_trace("SlotTarget = " + std::to_string(m_Slot) +
				"; SlotNow = " + std::to_string(slot_num) +
				"; sec = " + std::to_string(m_Clock.seconds(60, m_Fudge)) +
				"; thisSlot = " + std::to_string(thisSlot));*/
		}
		if (thisSlot) {
	...
	if (m_Loop && count && decoder) {
		if (m_Rate == 12000) {
			// copy data into decode module
			if ((ct = decoder->write(out, count)) != count) {
				KK5JY::FT8::send_trace(
					"Monitor write returned " + std::to_string(ct) +
			...
			ct = decimate_buffer(out, count, m_LoopBuffer);

			// copy data into decode module
			decoder->write(m_LoopBuffer, ct);
		}
	}
}
...
void ModemSoundDevice::udp_audio(uint8_t *raw_data, size_t bytes) {
	...
	// DEBUG: use this only for debugging; it is very chatty
	// KK5JY::FT8::send_trace("UDP got " + std::to_string(bytes) + " bytes, " + std::to_string(samples) + " samples.");
	...
	for (size_t i = 0; i != samples; ++i) {
		sample = *ip++;           // input is S16LE, lsbtye first
		sample |= (*ip++ << 8);   // then msbyte
		*op++ = static_cast<float>(sample) / 32768.0;
	}

	// call the sound event handler
	event(m_NetBuffer, 0, samples);
}
