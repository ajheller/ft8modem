...
namespace KK5JY {
	namespace FT8 {
		...
		inline bool Decode<T>::startDecode() {
			...
			size_t full_frame = 0;
			if (m_Mode == "ft8")
				full_frame = 14.9 * JT9_RATE;
			else if (m_Mode == "ft4")
				full_frame = 7.4  * JT9_RATE;
			else if (m_Mode == "jt65")
				full_frame = 60.0 * JT9_RATE;
			else if (m_Mode == "jt9")
				full_frame = 60.0 * JT9_RATE;
			else if (m_Mode == "wspr")
				full_frame = 120.0 * WSPR_RATE;
		...
		inline static bool isValidDecode(const std::string &line) {
			if (line.size() < 4)
				return false;
			if (isdigit(line[0]) && isdigit(line[1])) // ft8, ft4, jt65
				return true;
			...
			try {
				...
				if (is_jt9) {
					...
					if (decode->m_Threads >= 2) {
						cmd += " -m ";
						cmd += std::to_string(decode->m_Threads);
					}
				}
				...
				while (jt9 /*&& ! feof(jt9)*/) {
					int ct = fread(iobuffer, 1, bufsz, jt9);
					...
					if (ct <= 0) {
						...
						// otherwise append whatever data was read
